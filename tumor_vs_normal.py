# -*- coding: utf-8 -*-
"""Tumor vs Normal.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XC8CNp2llpwRAl_IrxKZ8FZiP1CeoQSs
"""
...
data_dir = '/content/drive/MyDrive/MLClivusProject/CompleteDataset/TumorsVsNormals/Axial'
#can replace axial with sagittal/coronal images and run same code
...
class_names = full_dataset.classes  # ['Normal', 'PitNET']
...
class_weights = torch.tensor([1.0, 1.0], dtype=torch.float).to(device)  # Optional: compute real weights if unbalanced
...
normal_samples = random.sample([s for s in val_dataset if s[1] == 0], 5)
pa_samples = random.sample([s for s in val_dataset if s[1] == 1], 5)
fig, axes = plt.subplots(2, 5, figsize=(20, 8))

for i in range(5):
    for row, (samples, label_name) in enumerate(zip([normal_samples, pa_samples], ['Normal', 'PitNET'])):
...
plt.suptitle("Grad-CAM: Normal (Top) vs PitNET (Bottom)", fontsize=16)
...
plt.title('Receiver Operating Characteristic (ROC) - PitNET vs Normal')
